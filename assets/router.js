...
function parseHash() {
  ...
  if (path === 'roadmap' && maybeId) return { route: '/roadmap', params: { id: maybeId } };
...
function renderLanguages() {
  const items = StudyDevData.languages.filter(lang => lang.hello_world).map(lang => languageCard(lang)).join('');
  return `
    <section class="stack-md">
      ...
      <div class="stack-sm">
        <button id="load-more-btn" class="btn btn--ghost">Mostrar mais</button>
      </div>
    </section>
  `;
}
...
function renderLanguageDetail({ id }) {
  const lang = StudyDevData.languages.find(l => l.id === id);
  if (!lang) return renderNotFound();
  const about = (StudyDevData.aboutDetails && StudyDevData.aboutDetails[lang.id]) || lang.about;
...
function renderRoadmaps() {
  const items = StudyDevData.roadmaps.map(roadmap => roadmapCard(roadmap)).join('');
  return `
    <section class="stack-md">
...
function renderRoadmapDetail({ id }) {
  const roadmap = StudyDevData.roadmaps.find(r => r.id === id);
  if (!roadmap) return renderNotFound();

  const contentHtml = roadmap.content.map(item => {
    if (item.type === 'text') {
...
function renderBot() {
  const options = (StudyDevData.languages || [])
    .filter(l => l.hello_world)
      ...
      <div class="grid grid--responsive">
        <div class="card">
          ...
          <div class="stack-sm">
            ...
            <textarea id="bot-editor" class="editor" rows="12" placeholder="Cole seu código aqui..."></textarea>
            <div class="card__actions">
  ...
  return `
    <section class="stack-md">
      ...
      <div class="card">
        ...
        <div class="card__actions">
          <a href="mailto:contato@studydev.local" class="btn btn--ghost">E-mail</a>
