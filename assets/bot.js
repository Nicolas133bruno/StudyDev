...
`,
  ...
  analyze(code, lang) {
    ...
    // Passo 1: Detectar variáveis declaradas e usadas
    lines.forEach((line, i) => {
      const t = line.trim();
      const isControl = /^(if|for|while|switch|function|class)\b/.test(t);
      const isBlockEnd = /[{}]$/.test(t);

      // Detecção de variáveis declaradas (let/const)
      const varDeclare = /\b(let|const)\s+([a-zA-Z_$][a-zA-Z0-9_$]*)/.exec(t);
      if (varDeclare) {
        const [, type, name] = varDeclare;
        declaredVars.set(name, { line: i + 1, type, reassigned: false });
      }

      // Detecção de reatribuição de variáveis let
      const varReassign = /\b([a-zA-Z_$][a-zA-Z0-9_$]*)\s*=/.exec(t);
      if (varReassign) {
        const name = varReassign[1];
        if (declaredVars.has(name) && declaredVars.get(name).type === 'let') {
          declaredVars.set(name, { ...declaredVars.get(name), reassigned: true });
        }
      }

      // Detecção de uso de variáveis
      declaredVars.forEach((_, name) => {
        if (new RegExp(`\b${name}\b`).test(t) && !/\b(let|const)\s+${name}\b/.test(t)) {
          usedVars.add(name);
        }
      });
    ...
    // Passo 2: Adicionar sugestões para variáveis não utilizadas e let que podem ser const
    declaredVars.forEach((data, name) => {
      if (!usedVars.has(name)) {
        issues.push(`Linha ${data.line}: variável '${name}' declarada mas não utilizada.`);
      }
      if (data.type === 'let' && !data.reassigned) {
        suggestions.push(`Linha ${data.line}: variável '${name}' pode ser substituída por 'const' (não é reatribuída).`);
      }
    });
  ...
  fix(code) {
    ...
    // Passo 1: Detectar variáveis let não reatribuídas para converter em const
    const declaredVars = new Map();
    out.forEach((line, i) => {
      const t = line.trim();
      const varDeclare = /\b(let|const)\s+([a-zA-Z_$][a-zA-Z0-9_$]*)/.exec(t);
      if (varDeclare) {
        const [, type, name] = varDeclare;
        declaredVars.set(name, { lineIndex: i, type, reassigned: false });
      }
      const varReassign = /\b([a-zA-Z_$][a-zA-Z0-9_$]*)\s*=/.exec(t);
      if (varReassign) {
        const name = varReassign[1];
    ...
    // Passo 2: Converter let em const onde aplicável
    declaredVars.forEach((data, name) => {
      if (data.type === 'let' && !data.reassigned) {
        const line = out[data.lineIndex];
        out[data.lineIndex] = line.replace(/\blet\s+${name}\b/, `const ${name}`);
      }
    });

    // Passo 3: Verificações existentes de ponto e vírgula
    out = out.map(line => {
      const t = line.trim();
      const isControl = /^(if|for|while|switch|function|class)\b/.test(t);
      const isBlockEnd = /[{}]$/.test(t);
  ...
  registerLanguage('python', {
    analyze(code) {
      ...
      lines.forEach((line, i) => {
        ...
        // Detecção de variáveis declaradas (simples, pode ser aprimorado)
        const varDeclare = /\b([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*[^=]/.exec(t);
        if (varDeclare) {
          const name = varDeclare[1];
          declaredVars.set(name, { line: i + 1, name });
        }

        // Detecção de uso de variáveis
        declaredVars.forEach((data, name) => {
          if (new RegExp(`\b${name}\b`).test(t) && !/\b(def|class)\s+${name}\b/.test(t) && !/\b${name}\s*=/.test(t)) {
            usedVars.add(name);
          }
        });
        ...
        // Sugestão: snake_case para variáveis
        const camelCaseVar = /\b([a-z]+[A-Z][a-zA-Z0-9_]*)\b/.exec(t);
        if (camelCaseVar && !/^(def|class)\b/.test(t)) {
          suggestions.push(`Linha ${i + 1}: variável '${camelCaseVar[1]}' não está em snake_case. Considere renomear para '${camelCaseVar[1].replace(/[A-Z]/g, letter => `_${letter.toLowerCase()}`)}'.`);
        }
      });

      // Adicionar sugestões para variáveis não utilizadas
      declaredVars.forEach((data, name) => {
        if (!usedVars.has(name)) {
          issues.push(`Linha ${data.line}: variável '${name}' declarada mas não utilizada.`);
        }
    ...
    fix(code) {
      let out = code
        .replace(/\t/g, '    ') // Substitui tabulações por 4 espaços
        .split(/\r?\n/);

      out = out.map(line => {
        const t = line.trim();
        // Adiciona ':' se estiver faltando em declarações de bloco
        if (/^(def|if|for|while|class)\b/.test(t) && !/:$/.test(t)) return line + ':';
        // Converte print statement para print function (Python 3)
  ...
  registerLanguage('java', {
    analyze(code) {
      ...
      lines.forEach((line, i) => {
        ...
        // Detecção de variáveis declaradas (simples, pode ser aprimorado)
        const varDeclare = /\b(int|String|double|float|boolean|char)\s+([a-zA-Z_$][a-zA-Z0-9_$]*)\s*(=|;)/.exec(t);
        if (varDeclare) {
          const name = varDeclare[2];
          declaredVars.set(name, { line: i + 1, name });
        }

        // Detecção de uso de variáveis
        declaredVars.forEach((data, name) => {
          if (new RegExp(`\b${name}\b`).test(t) && !/\b(int|String|double|float|boolean|char)\s+${name}\b/.test(t) && !/\b${name}\s*=/.test(t)) {
            usedVars.add(name);
          }
        });
        ...
        // Sugestão: camelCase para variáveis
        const snakeCaseVar = /\b([a-z]+_[a-zA-Z0-9_]*)\b/.exec(t);
        if (snakeCaseVar) {
          suggestions.push(`Linha ${i + 1}: variável '${snakeCaseVar[1]}' não está em camelCase. Considere renomear para '${snakeCaseVar[1].replace(/_([a-z])/g, g => g[1].toUpperCase())}'.`);
        }
      });
  ...
  registerLanguage('cpp', {
    analyze(code) {
      ...
      lines.forEach((line, i) => {
        ...
        // Detecção de variáveis declaradas (int, float, double, char, bool, string)
        const varDeclare = /\b(int|float|double|char|bool|string)\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*[=;]/.exec(t);
        if (varDeclare) {
          const [, type, name] = varDeclare;
          declaredVars.set(name, { line: i + 1, type, reassigned: false });
        }

        // Detecção de reatribuição de variáveis
        const varReassign = /\b([a-zA-Z_][a-zA-Z0-9_]*)\s*=/.exec(t);
        if (varReassign) {
          const name = varReassign[1];
        ...
        // Detecção de uso de variáveis
        declaredVars.forEach((data, name) => {
          if (new RegExp(`\\b${name}\\b`).test(t) && !/\\b(int|float|double|char|bool|string)\\s+${name}\\b/.test(t) && !/\\b${name}\\s*=/.test(t)) {
            usedVars.add(name);
          }
        });
      ...
      // Adicionar sugestões para variáveis não utilizadas e que podem ser const
      declaredVars.forEach((data, name) => {
        if (!usedVars.has(name)) {
          issues.push(`Linha ${data.line}: variável '${name}' declarada mas não utilizada.`);
        }
        if (!data.reassigned) {
          suggestions.push(`Linha ${data.line}: variável '${name}' pode ser declarada como 'const' (não é reatribuída).`);
        }
      });
    ...
    fix(code) {
      let out = code.split(/\r?\n/);
      const declaredVars = new Map();

      // Passo 1: Detectar variáveis para const e não utilizadas
      out.forEach((line, i) => {
        const t = line.trim();
        const varDeclare = /\b(int|float|double|char|bool|string)\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*[=;]/.exec(t);
        if (varDeclare) {
      ...
      // Passo 2: Converter para const e remover não utilizadas
      declaredVars.forEach((data, name) => {
        // Lógica de remoção de variáveis não utilizadas é complexa para regex simples,
        // então focaremos na conversão para const por enquanto.
        if (!data.reassigned) {
          const line = out[data.lineIndex];
          out[data.lineIndex] = line.replace(new RegExp(`\\b${data.type}\\s+${name}\\b`), `const ${data.type} ${name}`);
        }
      });
  ...
  registerLanguage('swift', {
    analyze(code) {
      const issues = [];
      const suggestions = [];
      if (!/func\s+main\s*\(\)/.test(code) && !/print\s*\(/.test(code)) suggestions.push('Adicione a função func main() ou um comando print.');
      if (/print\s+"[^"]+"/.test(code) && !/print\s*\(/.test(code)) issues.push('Use print("texto") com parênteses.');
      return { issues, suggestions };
    },
    fix(code) {
      let out = code;
      if (!/func\s+main\s*\(\)/.test(out) && !/print\s*\(/.test(out)) {
        const wrapped = out.split(/\r?\n/).map(l => '  ' + l).join('\n');
        out = `func main() {\n${wrapped}\n}\n`;
      }
      out = out.replace(/(print\s+)"([^"]+)"/g, 'print("$2")');
      return out;
    }
  });
  ...
  registerLanguage('html', {
    analyze(code) {
      const issues = [];
      const suggestions = [];
      const unmatchedAngles = /<[^>]*$/m.test(code);
      if (unmatchedAngles) issues.push('Tag possivelmente sem > final.');
      const selfClosing = new Set(['br','hr','img','input','meta','link','source','area','base','col','embed','param','track','wbr']);
      const stack = [];
      const tagRx = /<\/?([a-zA-Z][a-zA-Z0-9:-]*)([^>]*)>/g;
      let m;
      while ((m = tagRx.exec(code))) {
        const name = m[1].toLowerCase();
        const closing = m[0][1] === '/';
        const selfCloseAttr = /\/$/.test(m[0]) || /\/(\s*)>$/.test(m[0]);
        if (closing) {
          if (stack.length === 0 || stack[stack.length - 1] !== name) issues.push(`Tag de fechamento inesperada </${name}>.`);
          else stack.pop();
        } else if (!selfClosing.has(name) && !selfCloseAttr) {
          stack.push(name);
        }
      }
      if (stack.length) issues.push(`Tags não fechadas: ${stack.join(', ')}.`);
      const attrQuoteIssue = /<[a-zA-Z][^>]*=([^"'\s][^>\s]*)/m.test(code);
      if (attrQuoteIssue) suggestions.push('Use aspas em valores de atributos.');
      return { issues, suggestions };
    },
    fix(code) {
      return code;
  ...
  registerLanguage('css', {
    analyze(code) {
      ...
      if (open !== close) issues.push('Quantidade de { e } não balanceada.');
      const commentOpen = (code.match(/\/\*/g) || []).length;
      const commentClose = (code.match(/\*\//g) || []).length;
      if (commentOpen !== commentClose) issues.push('Comentário CSS não fechado.');
      const lines = code.split(/\r?\n/);
      lines.forEach((line, i) => {
        const t = line.trim();
        if (!t) return;
        if (/^@/.test(t) && /;\s*$/.test(t) === false && /\{\s*$/.test(t) === false) suggestions.push(`Linha ${i + 1}: considere finalizar @regra com ;.`);
        if (/:[^;}{]+$/.test(t) && /\{\s*$/.test(t) === false) suggestions.push(`Linha ${i + 1}: declaração pode estar sem ;.`);
      });
      return { issues, suggestions };
    },
    fix(code) {
  ...
  function attachBotIfPresent() {
    ...
    const editorEl = document.getElementById('bot-editor');
    ...
    if (!editorEl || !langSel || !btnAnalyze || !btnFix || !btnReset) return;

    let monacoEditor = null;
    let fallbackTextarea = null;
    function getUserId() {
      const k = 'studydev_user_id';
      let v = localStorage.getItem(k);
      if (!v) { v = Math.random().toString(36).slice(2); localStorage.setItem(k, v); }
      return v;
    }
    const limiter = (() => {
      const state = { global: [], users: {} };
      const cfg = { windowMs: 10000, maxGlobal: 5, userWindowMs: 10000, maxUser: 3 };
      function now() { return Date.now(); }
      function clean(arr, win) { const t = now(); return arr.filter(x => t - x < win); }
      function canProceed(userId) {
        state.global = clean(state.global, cfg.windowMs);
        const g = state.global.length < cfg.maxGlobal;
        state.users[userId] = clean(state.users[userId] || [], cfg.userWindowMs);
        const u = (state.users[userId] || []).length < cfg.maxUser;
        return g && u;
      }
      function record(userId) {
        ...
        state.users[userId].push(t);
      }
      return { canProceed, record };
    })();
    function traceRunStart(name, data) {
      try {
        const cfg = window.LANGSMITH_CONFIG || null;
        if (!cfg || !cfg.apiKey) return null;
        const body = { name, start_time: new Date().toISOString(), inputs: data };
        return fetch((cfg.endpoint || 'https://api.smith.langchain.com') + '/runs', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-api-key': cfg.apiKey }, body: JSON.stringify(body) }).then(r => r.ok ? r.json() : null).catch(() => null);
      } catch (_) { return null; }
    }
    function traceRunEnd(runId, data) {
      try {
        const cfg = window.LANGSMITH_CONFIG || null;
        if (!cfg || !cfg.apiKey || !runId) return;
        const body = { end_time: new Date().toISOString(), outputs: data };
        fetch((cfg.endpoint || 'https://api.smith.langchain.com') + '/runs/' + runId, { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'x-api-key': cfg.apiKey }, body: JSON.stringify(body) }).catch(() => {});
      } catch (_) {}
    }
    function mapMonacoLanguage(id) {
      if (id === 'javascript' || id === 'typescript') return id;
    ...
    function setEditorValue(v) {
      ...
      else if (fallbackTextarea) fallbackTextarea.value = v;
    }

    if (window.require) {
      const initLang = mapMonacoLanguage(langSel.value);
      window.require(['vs/editor/editor.main'], function() {
        monacoEditor = monaco.editor.create(editorEl, { value: getSample(langSel.value) || '', language: initLang, theme: 'vs-dark', automaticLayout: true, minimap: { enabled: false } });
        langSel.addEventListener('change', () => {
          const newLang = mapMonacoLanguage(langSel.value);
        ...
        monacoEditor.onDidChangeModelContent(() => {
          ...
          debounceTimer = setTimeout(() => {
            ...
            const base = analyze(getEditorValue(), lang);
            const formatItem = (item) => typeof item === 'object' && item !== null ? (item.message || item.text || JSON.stringify(item)) : String(item);
            issuesEl.innerHTML = base.issues.length ? base.issues.map(i => `<li class="issue">${formatItem(i)}</li>`).join('') : '<li class="muted">Nenhum problema crítico encontrado.</li>';
            suggestionsEl.innerHTML = base.suggestions.length ? base.suggestions.map(s => `<li class="suggestion">${formatItem(s)}</li>`).join('') : '<li class="muted">Sem recomendações adicionais.</li>';
          }, 300);
        });
      });
    } else {
      fallbackTextarea = document.createElement('textarea');
      fallbackTextarea.className = 'editor';
      fallbackTextarea.rows = 12;
      fallbackTextarea.placeholder = 'Cole seu código aqui...';
      fallbackTextarea.style.width = '100%';
      fallbackTextarea.style.height = '100%';
      editorEl.appendChild(fallbackTextarea);
      fallbackTextarea.value = getSample(langSel.value) || '';
      let debounceTimer = null;
      ...
      langSel.addEventListener('change', () => { setSample(); });
    }
    ...
    function setSample() {
      const lang = langSel.value;
      setEditorValue(getSample(lang) || getEditorValue() || '');
      issuesEl.innerHTML = '';
    ...
    try {
      const savedKey = localStorage.getItem('studydev_ai_key');
      if (savedKey && typeof savedKey === 'string' && savedKey.trim().length) AI_CONFIG.apiKey = savedKey.trim();
      const savedModel = localStorage.getItem('studydev_ai_model');
      if (savedModel && typeof savedModel === 'string' && savedModel.trim().length) AI_CONFIG.model = savedModel.trim();
    } catch (_) {}
    
    // Captura via URL: #/bot?ai_key=XXXX ou ?ai_key=XXXX
    (function captureKeyFromUrl(){
      try {
        let key = '';
        const url = new URL(window.location.href);
        key = url.searchParams.get('ai_key') || '';
        const idx = window.location.hash.indexOf('?');
        if (!key && idx !== -1) {
          const qs = new URLSearchParams(window.location.hash.slice(idx + 1));
          key = qs.get('ai_key') || '';
        }
        if (key && typeof key === 'string' && key.trim().length) {
          const clean = key.trim();
          try { localStorage.setItem('studydev_ai_key', clean); } catch (_) {}
          AI_CONFIG.apiKey = clean;
          if (apiKeyInput) apiKeyInput.value = clean;
          if (statusEl) statusEl.textContent = '🔑 Chave da IA aplicada via URL.';
          // limpa o parâmetro do hash para não deixar a chave exposta na barra
          if (idx !== -1) {
            window.location.hash = window.location.hash.slice(0, idx);
          }
        }
      } catch (_) {}
    })();
    function normalizeModelName(m) {
      ...
      s = s.replace(/gemmini/g, 'gemini').replace(/,/g, '.');
      return s || 'gemini-1.5-flash';
    }
    async function pickAvailableModel(preferRaw) {
      const prefer = normalizeModelName(preferRaw);
      ...
      return { version: 'v1', model: prefer };
    }
    async function aiAnalyze(language, code) {
      ...
      if (!limiter.canProceed(userId)) {
        throw new Error('429: limite de requisições atingido. Tente novamente em instantes.');
      }
      limiter.record(userId);
      const runStart = traceRunStart('analyze_fix', { language, size: code.length });
      if (AI_CONFIG && AI_CONFIG.provider === 'gemini_direct') {
        ...
        if (res.status === 429) {
          ...
          res = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contents: [{ role: 'user', parts: [{ text: prompt }] }] }) });
        }
        ...
        Promise.resolve(runStart).then(r => traceRunEnd(r && r.id, { issues: out.issues.length, suggestions: out.suggestions.length }));
        return out;
      }
      ...
      let res = await fetch(AI_CONFIG.endpoint, {
        method: 'POST',
        headers,
      ...
      if (res.status === 429) {
        await new Promise(r => setTimeout(r, 1000));
        res = await fetch(AI_CONFIG.endpoint, { method: 'POST', headers, body: JSON.stringify({ provider: AI_CONFIG.provider, language: aiLang, code, task: 'analyze_fix', prompt: AI_CONFIG.prompt || '' }) });
      }
    ...
    btnAnalyze.onclick = async () => {
      ...
      if (AI_CONFIG && AI_CONFIG.dataJsPath && lang === 'javascript' && (!getEditorValue() || !getEditorValue().trim())) {
        try {
      ...
      issuesEl.innerHTML = issues.length ? issues.map(i => `<li class="issue">${formatItem(i)}</li>`).join('') : '<li class="muted">Nenhum problema crítico encontrado.</li>';
      suggestionsEl.innerHTML = suggestions.length ? suggestions.map(s => `<li class="suggestion">${formatItem(s)}</li>`).join('') : '<li class="muted">Sem recomendações adicionais.</li>';
      if (statusEl) {
    ...
    btnFix.onclick = async () => {
      ...
      if (issues.length === 0) {
        ...
        return;
            ...
            const proceed = window.confirm('Foram encontrados problemas. Deseja aplicar as correções sugeridas pela IA?');
      if (!proceed) {
      ...
      try {
        ...
        const ai = await aiAnalyze(lang, getEditorValue());
        const fixed = (typeof ai.fixed_code === 'string') ? ai.fixed_code : getEditorValue();
        const localFixed = StudyBot.fix(getEditorValue(), lang);
        const aiFixedIsDifferent = fixed.trim() !== getEditorValue().trim();
        const localFixedIsDifferent = localFixed.trim() !== getEditorValue().trim();

        if (aiFixedIsDifferent) {
        ...
        } else if (localFixedIsDifferent) {
          outputEl.textContent = localFixed;
          if (statusEl) {
            statusEl.textContent = '🛠️ IA não gerou mudanças. Apliquei correções locais básicas.';
            statusEl.setAttribute('aria-busy', 'false');
          }
        } else {
          outputEl.textContent = getEditorValue();
          if (statusEl) {
            statusEl.textContent = 'ℹ️ Nenhuma correção necessária ou gerada.';
            statusEl.setAttribute('aria-busy', 'false');
          }
        }
      } catch (err) {
        ...
        const localFixed = StudyBot.fix(getEditorValue(), lang);
        const localFixedIsDifferent = localFixed.trim() !== getEditorValue().trim();
        outputEl.textContent = localFixedIsDifferent ? localFixed : getEditorValue();
        if (statusEl) {
          statusEl.textContent = localFixedIsDifferent
            ? '🛠️ IA indisponível. Correções locais aplicadas.'
            : 'ℹ️ IA indisponível. Correção não aplicada.';
          statusEl.setAttribute('aria-busy', 'false');
        }
      }
      btnFix.disabled = false; btnAnalyze.disabled = false;
    ...
    if (btnCopy) {
      btnCopy.onclick = async () => {
        const text = outputEl.textContent || getEditorValue() || '';
        try {
